...
def _draw_text_data_coord2(height_matrix, ax, fontfamily,
                             colorscheme='classic', scalex=1, draw_axis=False, debug=False):
    fig = ax.get_figure()
...
def _draw_text_data_coord(height_matrix, ax, fontfamily,
                             colorscheme='classic', scalex=1, draw_axis=False, debug=False):
    fig = ax.get_figure()
    bbox = ax.get_window_extent().transformed(fig.dpi_scale_trans.inverted())
    width, height = bbox.width, bbox.height
...
def _draw_text_display_coord2(height_matrix, ax, fontfamily,
                          colorscheme='classic', scalex=1, draw_axis=False, is_protein=False):
    xshifts_list = []
    fig = ax.get_figure()
    bbox = ax.get_window_extent().transformed(fig.dpi_scale_trans.inverted())
    width, height = bbox.width, bbox.height
...
def _draw_text_display_coord(height_matrix, ax, fontfamily,
               colorscheme='classic', scalex=1, is_protein=False, draw_axis=False):
    xshifts_list = []
    fig = ax.get_figure()
    bbox = ax.get_window_extent().transformed(fig.dpi_scale_trans.inverted())
    width, height = bbox.width, bbox.height
...
def draw_protein(data, ax, data_type='bits', seq_type='dna',
    ...
    _draw_text_display_coord(data, ax, fontfamily, colorscheme, scalex=scalex, is_protein=True)
...
def draw_logo(data, data_type='bits', seq_type='dna',
    ...
    if coordinate_type == 'data':
        ...
        ax.set_xticklabels(range(1, len(data) + 1), rotation=90)
        if yaxis == 'probability':
            xshifts_list = _draw_text_data_coord(pfm, ax, fontfamily, colorscheme, draw_axis=draw_axis)
        else:
            xshifts_list = _draw_text_data_coord(ic, ax, fontfamily, colorscheme, draw_axis=draw_axis)
    else:
        if yaxis == 'probability':
            xshifts_list = _draw_text_display_coord(pfm, ax, fontfamily, colorscheme, draw_axis=draw_axis)
        else:
            xshifts_list = _draw_text_display_coord(ic, ax, fontfamily, colorscheme, draw_axis=draw_axis)
    ...
    for i in range(nrow):
        for j in range(ncol):
            if i==j==0:
                ...
                continue
            despine(ax=axarr[i, j], trim=False, top=True, right=True, bottom=True, offset=160)
            axi = axarr[i, j]
